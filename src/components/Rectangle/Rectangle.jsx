...
export const Rectangle = ({ stateProp, className, taskk }) => {
  ...
  return (
      ...
      {data.map((k) => {
        ...
        return shouldRender ? (
          <>
            <div
              key={k.id}
              className={` bg-transparent w-[270px] relative  px-[7px] overflow-hidden rounded-[8px]
    ${state.state === 'default' ? 'flex' : ''} ${
                state.state === 'default' ? 'flex-col' : ''
              } ${state.state === 'default' ? 'items-start' : ''} ${
    ...
    ${
     ...
     ${state.state === 'default' ? 'p-[16px]' : ''} ${
                state.state === 'hover' ? 'h-[114px]' : ''
              } ${className}
     `}
            >
              <div
                ...
                } ${state.state === 'hover' ? 'left-[16px]' : ''} ${
                  state.state === 'default' ? 'mt-[-1.00px]' : ''
              ...
              <div
                className={` inline-flex items-start gap-[24px] ${
                  state.state === 'hover' ? 'left-[16px]' : ''
                } ${state.state === 'hover' ? 'top-[50px]' : ''} ${
                  state.state === 'default' ? 'flex-[0_0_auto]' : ''
                } ${state.state === 'hover' ? 'absolute' : 'relative'}`}
              >
                <DataLabel
